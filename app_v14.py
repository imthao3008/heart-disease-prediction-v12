...
def preprocess_input(user_input, imp_num, imp_cat, scaler, num_cols, other_cols, all_columns):
    ...
    # Align columns với tập train
    row = row.reindex(columns=all_columns, fill_value=0)

    # ── Impute num: PHẢI dùng đúng num_cols (6 cột) như lúc fit ──────────────
    # imp_num fit trên num_cols = ['age','trestbps','chol','thalch','oldpeak','ca']
    # Lấy đúng 6 cột theo thứ tự num_cols, convert sang numpy để bỏ feature check
    arr_num = np.array([[row[c].values[0] for c in num_cols]], dtype=float)
    imputed_num = imp_num.transform(arr_num)[0]
    for i, c in enumerate(num_cols):
        row[c] = imputed_num[i]

    # ── Impute cat: các cột còn lại ──────────────────────────────────────────
    oc = [c for c in row.columns if c not in num_cols]
    if oc:
        arr_cat = np.array([[row[c].values[0] for c in oc]], dtype=float)
        imputed_cat = imp_cat.transform(arr_cat)[0]
        for i, c in enumerate(oc):
            row[c] = imputed_cat[i]

    # ── Scale: đúng num_cols ─────────────────────────────────────────────────
    row_sc = row.copy()
    arr_sc = np.array([[row[c].values[0] for c in num_cols]], dtype=float)
    scaled = scaler.transform(arr_sc)[0]
    for i, c in enumerate(num_cols):
        row_sc[c] = scaled[i]
