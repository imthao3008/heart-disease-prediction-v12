...
def preprocess_input(user_input, imp_num, imp_cat, scaler, num_cols, other_cols, all_columns):
    ...
    # Impute — dùng đúng cột num_cols gốc (imp_num được fit với num_cols)
    nc = [c for c in num_cols if c in row.columns]
    oc = [c for c in row.columns if c not in nc]

    # Transform riêng từng cột để tránh lỗi shape mismatch
    if nc:
        row_num = row[nc].copy()
        row[nc] = imp_num.transform(row_num)
    if oc:
        row_cat = row[oc].copy()
        row[oc] = imp_cat.transform(row_cat)
...
with st.sidebar:
    ...
    if input_mode == "\U0001f5c3\ufe0f Ch\u1ecdn t\u1eeb t\u1eadp Test UCI":
        ...
        st.markdown('<div class="section-title">\U0001f4cb Th\u00f4ng tin b\u1ec7nh nh\u00e2n</div>',
                    unsafe_allow_html=True)
        ...
        fbs      = st.radio("D\u01b0\u1eddng huy\u1ebft l\u00fac \u0111\u00f3i > 120 mg/dL",
                            [False, True],
                            format_func=lambda x: "C\u00f3" if x else "Kh\u00f4ng")
        st.markdown("---")
        st.markdown('<div class="section-title">\U0001f52c K\u1ebft qu\u1ea3 x\u00e9t nghi\u1ec7m</div>',
                    unsafe_allow_html=True)
        ...
        slope_raw  = st.selectbox("Độ dốc ST khi gắng sức", slope_opts, index=0,
                                  help="Chọn 'Không thực hiện / Không có' nếu không có kết quả")
        ...
        ca_raw  = st.selectbox("Số nhánh mạch vành hẹp (ca)", ca_opts, index=0,
                               help="0–3 nhánh hẹp. Chọn 'Không thực hiện' nếu chưa chụp mạch vành")
        ...
        thal_raw  = st.selectbox("Kết quả Thalassemia", thal_opts, index=0,
                                 help="Chọn 'Không thực hiện' nếu chưa xét nghiệm Thalassemia")
